...
export const loginUser = userData => dispatch => {
  axios
    .post(`${constants.BASE_URL_PROD}/data/login`, userData)
    .then(userRes => {
...
export const createUser = userData => dispatch => {
  axios
    .post(`${constants.BASE_URL_PROD}/user/new`, userData)
    .then(userRes => {
      authSet(userRes.data.token);
      dispatch({
...
export const getBattleMessages = type => dispatch => {
  axios
    .get(`${constants.BASE_URL_PROD}/notifications/messages/${type}`)
    .then(battleMessages => {
      dispatch({
...
export const getLeaderBoardUsers = () => dispatch => {
  axios
    .get(`${constants.BASE_URL_PROD}/data/leaderboard`)
    .then(data => {
      dispatch({
...
export const handleBotBattle = () => async dispatch => {
  ...
  const botBattleSave = await axios
    .post(`${constants.BASE_URL_PROD}/notifications/battle`, {
      battleType: "bot"
    })
    .then(result => {
      success = true;
...
export const getPossibleMatchup = () => dispatch => {
  axios
    .get(`${constants.BASE_URL_PROD}/data/getBattleUsers`)
    .then(users => {
      dispatch({
...
export const getSquad = () => dispatch => {
  axios
    .get(`${constants.BASE_URL_PROD}/squad/get`)
    .then(res => {
      dispatch({
...
export const createSquad = squadData => async dispatch => {
  ...
  const handleCreations = await axios
    .post(`${constants.BASE_URL_PROD}/squad/create`, { squadName: squadData })
    .then(data => {
      console.log(data);
      success = true;
    })
...
export const getCmdBrdInfo = () => dispatch => {
  axios
    .get(`${constants.BASE_URL_PROD}/squad/command-panel-info`)
    .then(res => {
...
export const saveSquadChanges = squadChanges => async dispatch => {
  ...
  const saveSquadChanges = await axios
    .post(`${constants.BASE_URL_PROD}/squad/update-squad`, {
...
export const reqPasswordReset = email => async dispatch => {
  ...
  const sendReset = await axios
    .post(`${constants.BASE_URL_PROD}/data/reset/`, { email })
    .then(res => {
      console.log(res.data);
...
export const checkResetToken = paramToken => async dispatch => {
  ...
  const checkToken = await axios
    .get(`${constants.BASE_URL_PROD}/data/reset/${paramToken}`)
    .then(res => {
...
export const postPasswordReset = (passwords, paramToken) => async dispatch => {
  let success = false;
  const sendReset = await axios
    .post(`${constants.BASE_URL_PROD}/data/reset/${paramToken}`, {
...
export const searchForSquad = squadName => async dispatch => {
  ...
  const handleSearch = await axios
    .get(`${constants.BASE_URL_PROD}/squad/${squadName}`)
    .then(res => {
      console.log(res.data);
      response = res.data;
      dispatch({
...
export const sendJoinRequest = squadName => dispatch => {
  ...
  axios
    .post(`${constants.BASE_URL_PROD}/squad/join/request`, {
      squadName
    })
    .then(res => {
      console.log(res.data);
...
export const handleJoinRequestResponse = decisionData => async dispatch => {
  const handleDecision = await axios
    .post(`${constants.BASE_URL_PROD}/squad/handle-request`, {
...
export const removeUserFromSquad = users => dispatch => {
  axios
    .post(`${constants.BASE_URL_PROD}/squad/delete/members`, { userIds: users })
    .then(res => res.data)
    .catch(err => console.log(err));
};
...
export const squadInformationPanel = () => dispatch => {
  axios
    .get(`${constants.BASE_URL_PROD}/squad/squad-information/panel`)
    .then(res => {
      dispatch({
